...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            if (self.use_DIA):
                device = x.device
                self.attention3 = md.Attention(self.type, in_channels=self.channels)
    ...
    def __init__(
        ...
        for layer_idx in range(len(self.in_channels) - 1):
            for depth_idx in range(layer_idx+1):
                ...
                blocks[f'x_{depth_idx}_{layer_idx}'] = DecoderBlock(in_ch, skip_ch, out_ch, use_DIA, **kwargs)
        blocks[f'x_{0}_{len(self.in_channels)-1}'] =\
            DecoderBlock(self.in_channels[-1], 0, self.out_channels[-1], use_DIA, **kwargs)
