...
class EfficientUnet(nn.Module):
    def __init__(self, encoder, out_channels=3, concat_input=False):
        super().__init__()

        self.encoder = encoder
        self.concat_input = concat_input

        # [592, 296, 152, 80, 35, 32]
        self.up_conv1 = up_conv(self.n_channels, 512)
        self.att1 = Attention_block(80,512,258)
        self.double_conv1 = double_conv(self.size[0], 512)
        self.up_conv2 = up_conv(512, 256)
        self.att2 = Attention_block(40, 256, 128)
        self.double_conv2 = double_conv(self.size[1], 256)
        self.up_conv3 = up_conv(256, 128)
        self.att3 = Attention_block(24, 128, 64)
        self.double_conv3 = double_conv(self.size[2], 128)
        self.up_conv4 = up_conv(128, 64)
        self.att4 = Attention_block(16, 64, 32)
    ...
    def forward(self, x):
        ...
        x = self.up_conv1(x)
        a1 = self.att1(x, blocks.popitem()[1])
        x = torch.cat([x, a1], dim=1)
        x = self.double_conv1(x)

        x = self.up_conv2(x)
        a2 = self.att2(x, blocks.popitem()[1])
        x = torch.cat([x, a2], dim=1)
        x = self.double_conv2(x)

        x = self.up_conv3(x)
        a3 = self.att3(x, blocks.popitem()[1])
        x = torch.cat([x, a3], dim=1)
        x = self.double_conv3(x)

        x = self.up_conv4(x)
        a4 = self.att4(x, blocks.popitem()[1])
